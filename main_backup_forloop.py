...
#   Train data를 뽑는 옵션 (same : 모든 피실험자에서 동일한 수의 data를 train data로 추출, 1 : 피실험자를 고려하지 않고 정말 랜덤하게 train data로 추출)
...
#   Convolutional layer를 몇 개까지 늘려서 정확도를 확인할지 지정
conv_deep_range = np.arange(1, (3 + 1), 1)
...
conv_kernel_size_range = [1, 3, 5, 7]
...
#   화면상 구분을 위해 ('=======================' 이걸 긋는 효과)
print('=' * 100);
print('해당 실험과 관련된 폴더 내에 존재하는 모든 files의 name을 저장하고 있습니다.');
print()
...
print('전체 data files의 name은 : ');
print(total_files_name)

### 이번 deep learning에서 사용할 피실험자들 data 선택
##  File(딕셔너리 key)의 이름을 저장해놓은 list에서 추출하면 딕셔너리에서 추출하는 효과 발생
#   화면상 구분을 위해 ('=======================' 이걸 긋는 효과)
print('=' * 100);
print('이번 deep learning에서 사용할 markers와 피실험자들의 files name을 추출하고 있습니다.');
print()
...
#   화면상 구분을 위해 ('=======================' 이걸 긋는 효과)
print('=' * 100);
print('위에서 추출한 files name을 가지고 해당하는 files의 data만 불러오고 있습니다.');
print()
...
#   화면상 구분을 위해 ('=======================' 이걸 긋는 효과)
print('=' * 100);
print('불러온 data에서 input data와 target data를 나누고 있습니다.');
print()
...
#   화면상 구분을 위해 ('=======================' 이걸 긋는 효과)
print('=' * 100);
print('원하는 시퀀스 길이에 맞게 data를 편집하고 있습니다. (CNN의 input은 크기를 맞춰주어야 하기 때문에) ');
print()
...
#   화면상 구분을 위해 ('=======================' 이걸 긋는 효과)
print('=' * 100);
print('불러온 data의 순서를 원하는 방법으로 랜덤하게 섞고 있습니다.');
print()
...
print(shuf_total_files_name[(0 * subject_trial_num): (1 * subject_trial_num)])
print('첫번째 마커, 두번째 피실험자 : ');
print(shuf_total_files_name[(1 * subject_trial_num): (2 * subject_trial_num)])

### 원하는 train 비율만큼 train set을 만들고 나머지는 test set을 만들기 위해 train 비율만큼 선택된 file name 중 일부를 train set으로 지정 후 나머지는 test set으로 지정
#   화면상 구분을 위해 ('=======================' 이걸 긋는 효과)
print('=' * 100);
print('원하는 train 비율만큼 train set용 data와 test set용 data을 만들고 있습니다.');
print()
...
#   화면상 구분을 위해 ('=======================' 이걸 긋는 효과)
print('=' * 100);
print('Classification을 위해 target data를 바탕으로 label data를 만드는 중입니다.');
print()
...
print('Train label 은 : ');
print(train_label)
print('첫번째 train target 값은 : ');
print(train_target_set[0])
print('첫번째 train label 은 : ');
print(train_label[0])
...
print('Test label 은 : ');
print(test_label)
print('첫번째 test target 값은 : ');
print(test_target_set[0])
print('첫번째 test label 은 : ');
print(test_label[0])
...
##  반복 시행
for loop_deep in conv_deep_range:

    #   Convolutional layer의 수 (얼마나 deep한지를 결정)
    conv_layer_number = loop_deep

    #   최종 size와 개수의 구조를 만들 수 있도록 구한 kernel size와 개수의 조합을 저장할 리스트 변수 초기화
    total_size_cases = []
    total_num_cases = []

    #   입력한 size와 개수의 범위를 가지고, 입력한 deep과 width를 고려하여 나올 수 있는 width의 경우의 수를 다 구함
    for i in range(1, (conv_width_range[-1] + 1), 1):
        #   Width가 늘어날 때마다의 만들 수 있는 kernel size의 조합을 구함.
        now_size_cases = itertools.combinations(conv_kernel_size_range, i)
        now_size_cases = [list(j) for j in now_size_cases]
        #   Width가 늘어날 때마다의 만들 수 있는 kernel 개수의 조합을 구함.
        now_num_cases = itertools.combinations(conv_kernel_num_range, i)
        now_num_cases = [list(j) for j in now_num_cases]

        #   구한 kernel size와 개수의 조합을 합쳐서 최종 size와 개수의 구조를 만들 수 있게 준비
        total_size_cases = total_size_cases + now_size_cases
        total_num_cases = total_num_cases + now_num_cases

        #   Kernel의 size와 개수의 조합을 바탕으로 최종 size와 개수의 경우의 수를 구함
        total_size = itertools.product(total_size_cases, repeat=conv_layer_number)
        total_size = [list(j) for j in total_size]
        total_num = itertools.product(total_num_cases, repeat=conv_layer_number)
        total_num = [list(j) for j in total_num]

    # print('total_size') ; print(total_size)
    # print('total_num') ; print(total_num)

    #   구한 경우의 수를 바탕으로 실제 convolutional layer의 구조를 만드는데 사용할 입력에 맞도록 만들어주기
    for i in total_size:
        ...
        # print('conv_kernel_size은 : ', conv_kernel_size)
        # print('conv_kernel_size의 type : ', type(conv_kernel_size))
        # print('conv_kernel_size의 행 길이 : ', len(conv_kernel_size))

        #   해당 width 구조에서의 layer별 width의 크기를 계산
        #   Width 크기를 저장할 list 초기화
        now_width = []
        #   Width의 크기를 계산하여 저장
        for k in range(len(conv_kernel_size)):
            now_width = now_width + [len(conv_kernel_size[k])]

        # print('현재 구조에서의 width의 크기는 : ', now_width)

        ##  한 개의 convolutional layer에서 몇 개의 서로 다른 filter를 사용하여 특징을 뽑아낼 것인지 지정
        #   각 원소는 차례대로 각 layer에 존재하는 서로 다른 filter의 개수를 의미
        #   list(loop_width) : [1, 1], [1, 3] [1, 5]와 같은 list 형태
        conv_width_number = now_width

        print('conv_width_number : ', conv_width_number)

        ##  해당 구조에 맞는 kernel 개수를 생성
        #   해당 width 구조에 맞는 최종 개수의 구조를 만들 수 있도록 구한 kernel 개수의 경우의 수를 저장할 리스트 변수 초기화
        # total_num_cases = []
        #   해당 구조에 맞는 kernel 개수를 생성 후 저장

        if len(conv_width_number) == 1:
            now_num_cases_01 = itertools.product(conv_kernel_num_range, repeat=conv_width_number[0]);
            now_num_cases_01 = [list(j) for j in now_num_cases_01]

            total_num_cases = itertools.product(now_num_cases_01, repeat=1);
            total_num_cases = [list(j) for j in total_num_cases]

        elif len(conv_width_number) == 2:
            now_num_cases_01 = itertools.product(conv_kernel_num_range, repeat=conv_width_number[0]);
            now_num_cases_01 = [list(j) for j in now_num_cases_01]
            now_num_cases_02 = itertools.product(conv_kernel_num_range, repeat=conv_width_number[1]);
            now_num_cases_02 = [list(j) for j in now_num_cases_02]

            total_num_cases = itertools.product(now_num_cases_01, now_num_cases_02, repeat=1);
            total_num_cases = [list(j) for j in total_num_cases]

        elif len(conv_width_number) == 3:
            now_num_cases_01 = itertools.product(conv_kernel_num_range, repeat=conv_width_number[0]);
            now_num_cases_01 = [list(j) for j in now_num_cases_01]
            now_num_cases_02 = itertools.product(conv_kernel_num_range, repeat=conv_width_number[1]);
            now_num_cases_02 = [list(j) for j in now_num_cases_02]
            now_num_cases_03 = itertools.product(conv_kernel_num_range, repeat=conv_width_number[2]);
            now_num_cases_03 = [list(j) for j in now_num_cases_03]

            total_num_cases = itertools.product(now_num_cases_01, now_num_cases_02, now_num_cases_03, repeat=1);
            total_num_cases = [list(j) for j in total_num_cases]

        print('total_num_cases');
        print(total_num_cases)

        ##  개수에 대한 전체 경우의 수에서 하나씩 선택
        for k in total_num_cases:
            ...
            conv_kernel_number = k

            print('conv_kernel_size은 : ', conv_kernel_size)
            print('conv_kernel_num은 : ', conv_kernel_number)

            ##  전체 반복 중에서 몇 번째 인지 알기 위해 표시
            print();
            print('전체 ', str(len(conv_deep_range) * len(total_size) * len(total_num_cases)), '번의 반복 중에서 ', iter_order, '번째 반복을 진행 중입니다.')
            print('!!! 지금은 conv deep이 ', conv_layer_number, '이고, conv widht가 ', conv_width_number, '이고, kernel의 size가 ', conv_kernel_size, '이고, kernel의 개수가 ', conv_kernel_number, '일 때 입니다. !!!');
            print()
            ...
            #   화면상 구분을 위해 ('=======================' 이걸 긋는 효과)
            print('=' * 100);
            print('설정한 weight와 bias의 크기와 사이즈, layer의 깊이와 넓이에 맞게 초기화된 weight와 bias들을 생성하기 시작하였습니다.');
            print()
            ...
            #   화면상 구분을 위해 ('=======================' 이걸 긋는 효과)
            print('=' * 100);
            print('초기화된 weight와 bias들을 이용하여 원하는 CNN 구조를 생성하기 시작하였습니다.');
            print()
            ...
            print('=' * 100);
            print('형성된 CNN 구조를 이용하여 학습을 시작합니다.');
            print()
            ...
            print('Train set에 대한 setimated label의 type은 ; ');
            print(type(estimated_trainset_output))
            print('Train set에 대한 estimated label의 크기는 : ');
            print(estimated_trainset_output.shape)
            ...
            print('첫번째 train target 값은 : ');
            print(train_target_set[0])
            print('첫번째 estimated target value는 : ');
            print(estimated_train_target_value[0])
            '''
            ##  원래 값과 label을 다시 값으로 바꾼 대표값 사이의 차이 정도를 보기 위해 그래프 출력
            ...
            print('Test set에 대한 setimated label의 type은 ; ');
            print(type(estimated_testset_output))
            print('Test set에 대한 estimated label의 크기는 : ');
            print(estimated_testset_output.shape)
            ...
            print(train_target_set[0])
            print('첫번째 estimated target value는 : ');
            print(estimated_test_target_value[0])
            ...
            now_accuracy_name = 'deep_' + str(conv_layer_number) + '_width_' + str(conv_width_number) + '_size_' + str(conv_kernel_size) + '_num_' + str(conv_kernel_number)
            ...
            ##  csv 파일로 저장
            #   저장할 폴더명 생성
            save_folder_name = './' + experiment_name + '_fcdeep_' + str(fullyconnected_layer_number) + '_fcunit_' + str(fullyconnected_layer_unit[0]) + '_epoch_' + str(epoch_num)
            ...
            with open(save_file_name, 'at', newline='') as csvfile:
                ...
                # 맨 처음에만 열의 이름 부여
                if (iter_order == 1):
                    writer.writerow(['Conv deep', 'Conv width', 'Kernel size', 'Kernel num', 'Train accuracy', 'Test accuracy'])

                writer.writerow([str(conv_layer_number)] + [str(conv_width_number)] + [str(conv_kernel_size)] + [str(conv_kernel_number)] + save_file_data[0].tolist() + save_file_data[1].tolist())
            csvfile.close()

            ##  실제 마커 값과 예측한 마커값을 저장
            #   파일 이름 지정 (Train 경우)
            save_file_name = save_folder_name + '/convdeep_' + str(conv_layer_number) + '_convwidth_' + str(conv_width_number) + '_kernelsize_' + str(conv_kernel_size) + '_kernelnum_' + str(conv_kernel_number) + '_trainaccuracy_' + str(
                train_accuracies[-1])  # + '_testaccuracy_' + test_accuracies[-1]
            ...
            #   파일 이름 지정 (Test 경우)
            save_file_name = save_folder_name + '/convdeep_' + str(conv_layer_number) + '_convwidth_' + str(conv_width_number) + '_kernelsize_' + str(conv_kernel_size) + '_kernelnum_' + str(conv_kernel_number) + '_testaccuracy_' + str(test_accuracies[-1])  # + '_testaccuracy_' + test_accuracies[-1]
            # save_file_name_feature = save_file_name + '_featuremaps'
            save_file_name = save_file_name + '_estimation'
            save_file_name = save_file_name + '.csv'
            ...
            save_file_name = save_folder_name + '/convdeep_' + str(conv_layer_number) + '_convwidth_' + str(conv_width_number) + '_kernelsize_' + str(conv_kernel_size) + '_kernelnum_' + str(conv_kernel_number) + '_train_confusionmatrix' + '_normal'
            ...
            save_file_name = save_folder_name + '/convdeep_' + str(conv_layer_number) + '_convwidth_' + str(conv_width_number) + '_kernelsize_' + str(conv_kernel_size) + '_kernelnum_' + str(conv_kernel_number) + '_test_confusionmatrix' + '_normal'
